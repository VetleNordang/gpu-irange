...
__global__ void search(int SearchEF, std::string saveprefix, int edge_limit) {
    printf("searching with ef=%d, edge_limit=%d\n", SearchEF, edge_limit);
    // search process is implemented here, and the search result is saved to saveprefix
}
...
int main(int argc, char **argv) {
    ...
    search<<<1, 1>>>(SearchEF, paths["result_saveprefix"], M);
