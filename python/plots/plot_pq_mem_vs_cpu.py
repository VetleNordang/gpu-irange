#!/usr/bin/env python3
"""
Plot QPS and speedup for the parallel-neighbour serial-distance GPU version (GPU-PQ)
versus CPU-P on YT-Audio 1M and GIST-1M.

Produces two figures per dataset written to:
  executable_data/PQ_based_memory/analysis/

Usage:
  python python/plots/plot_pq_mem_vs_cpu.py
  python python/plots/plot_pq_mem_vs_cpu.py --dataset audi
  python python/plots/plot_pq_mem_vs_cpu.py --ranges 2,8 --env idun
  python python/plots/plot_pq_mem_vs_cpu.py --copy-to-thesis
"""
...
GPU_BASE    = BASE_DIR / "PQ_based_memory"
THESIS_FIGS = Path(__file__).resolve().parent.parent.parent / "thesis/Master/figs/design/gpu_pq_dev"
...
METHODS = [
    ...
    {"key": "gpu_pq_dev", "label": "GPU-PQ",  "color": "tab:orange"},
]
...
def plot_qps(cpu, gpu, active_ranges, out_path, title, hw_label, dataset_name):
    ...
    handles, labels = [], []
    drew = False
    for m, series in [("cpu_p", cpu), ("gpu_pq_dev", gpu)]:
...
def plot_speedup(cpu, gpu, active_ranges, out_path, title, hw_label, dataset_name):
    ...
    ax.set_xscale("log")
    ax.set_xlabel("SearchEF")
    ax.set_ylabel("Speedup  (GPU-PQ QPS / CPU-P QPS)")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend()
    if title:
        t = f"{dataset_name} — {hw_label}: GPU-PQ speedup over CPU-P" if hw_label \
            else f"{dataset_name}: GPU-PQ speedup over CPU-P"
        ax.set_title(t, fontsize=12, fontweight="bold")
    fig.tight_layout()
    _save(fig, out_path)
...
def main():
    p = argparse.ArgumentParser(
        description="Plot GPU-PQ (parallel neighbour, serial dist) vs CPU-P.")
    ...
    p.add_argument("--copy-to-thesis", action="store_true",
                   help="Copy output figures into thesis/Master/figs/design/gpu_pq_dev/.")
    args = p.parse_args()
    ...
    for d in datasets:
        ...
        qps_out     = out_dir / f"qps_pqmem_vs_cpu_{d['key']}{suffix}.png"
        speedup_out = out_dir / f"speedup_pqmem_vs_cpu_{d['key']}{suffix}.png"
