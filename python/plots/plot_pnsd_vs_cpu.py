#!/usr/bin/env python3
"""
Plot QPS and speedup for the parallel-neighbour serial-distance GPU version (GPU-v3)
versus CPU-P on YT-Audio 1M and GIST-1M.

Produces two figures per dataset written to:
  executable_data/parallel_neighbour_serial_dist/analysis/

Usage:
  python python/plots/plot_pnsd_vs_cpu.py
  python python/plots/plot_pnsd_vs_cpu.py --dataset audi
  python python/plots/plot_pnsd_vs_cpu.py --ranges 2,8 --env idun
  python python/plots/plot_pnsd_vs_cpu.py --copy-to-thesis
"""
...
GPU_BASE    = BASE_DIR / "parallel_neighbour_serial_dist"
THESIS_FIGS = Path(__file__).resolve().parent.parent.parent / "thesis/Master/figs/design/gpu_v3"

DATASETS = [
    {"key": "audi", "name": "YT-Audio 1M", "cpu_path": "audi/1m/results/cpu_parallel",        "cpu_prefix": "results"},
    {"key": "gist", "name": "GIST-1M",      "cpu_path": "gist1m/1000k/results/cpu_parallel",   "cpu_prefix": "results_1000k"},
]
...
METHODS = [
    ...
    {"key": "gpu_v3", "label": "GPU-v3",  "color": "tab:orange"},
]


def _parse_range(filename):
    m = re.search(r"results(?:_1000k)?(\d+)", filename)
    if not m:
        return None
    val = int(m.group(1))
...
def load_cpu(cpu_dir, active_ranges, prefix):
    agg_dir = cpu_dir / "aggregate"
    data = {}
    for rng in active_ranges:
        csv = agg_dir / f"{prefix}{rng}.csv"
        if csv.exists():
            df = pd.read_csv(csv)
...
def plot_qps(cpu, gpu, active_ranges, out_path, title, hw_label, dataset_name):
    ...
    handles, labels = [], []
    drew = False
    for m, series in [("cpu_p", cpu), ("gpu_v3", gpu)]:
...
def plot_speedup(cpu, gpu, active_ranges, out_path, title, hw_label, dataset_name):
    ...
    ax.set_xscale("log")
    ax.set_xlabel("SearchEF")
    ax.set_ylabel("Speedup  (GPU-v3 QPS / CPU-P QPS)")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend()
    if title:
        t = f"{dataset_name} — {hw_label}: GPU-v3 speedup over CPU-P" if hw_label \
            else f"{dataset_name}: GPU-v3 speedup over CPU-P"
        ax.set_title(t, fontsize=12, fontweight="bold")
    fig.tight_layout()
    _save(fig, out_path)
...
def main():
    p = argparse.ArgumentParser(
        description="Plot GPU-v3 (parallel neighbour, serial dist) vs CPU-P.")
    p.add_argument("--dataset", default=None,
                   help="Dataset key (audi, gist). Omit for all.")
    ...
    p.add_argument("--copy-to-thesis", action="store_true",
                   help="Copy output figures into thesis/Master/figs/design/gpu_v3/.")
    args = p.parse_args()
    ...
    for d in datasets:
        ...
        cpu = load_cpu(BASE_DIR / d["cpu_path"], active_ranges, d["cpu_prefix"])
        if not gpu and not cpu:
            print("  no data found — skipping")
            continue

        qps_out     = out_dir / f"qps_pnsd_vs_cpu_{d['key']}{suffix}.png"
        speedup_out = out_dir / f"speedup_pnsd_vs_cpu_{d['key']}{suffix}.png"
