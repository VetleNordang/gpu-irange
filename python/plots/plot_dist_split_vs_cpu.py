#!/usr/bin/env python3
"""
Plot QPS and speedup for the parallel-neighbour serial-distance GPU version (GPU-v4)
versus CPU-P on YT-Audio 1M and GIST-1M.

Produces two figures per dataset written to:
  executable_data/dist_split_4_threads/analysis/

Usage:
  python python/plots/plot_dist_split_vs_cpu.py
  python python/plots/plot_dist_split_vs_cpu.py --dataset audi
  python python/plots/plot_dist_split_vs_cpu.py --ranges 2,8 --env idun
  python python/plots/plot_dist_split_vs_cpu.py --copy-to-thesis
"""
...
GPU_BASE    = BASE_DIR / "dist_split_4_threads"
THESIS_FIGS = Path(__file__).resolve().parent.parent.parent / "thesis/Master/figs/design/gpu_v4"
...
METHODS = [
    ...
    {"key": "gpu_v4", "label": "GPU-v4",  "color": "tab:orange"},
]
...
def plot_qps(cpu, gpu, active_ranges, out_path, title, hw_label, dataset_name):
    ...
    handles, labels = [], []
    drew = False
    for m, series in [("cpu_p", cpu), ("gpu_v4", gpu)]:
...
def plot_speedup(cpu, gpu, active_ranges, out_path, title, hw_label, dataset_name):
    ...
    ax.set_xscale("log")
    ax.set_xlabel("SearchEF")
    ax.set_ylabel("Speedup  (GPU-v4 QPS / CPU-P QPS)")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend()
    if title:
        t = f"{dataset_name} — {hw_label}: GPU-v4 speedup over CPU-P" if hw_label \
            else f"{dataset_name}: GPU-v4 speedup over CPU-P"
        ax.set_title(t, fontsize=12, fontweight="bold")
    fig.tight_layout()
    _save(fig, out_path)
...
def main():
    p = argparse.ArgumentParser(
        description="Plot GPU-v4 (dist split 4 threads) vs CPU-P.")
    ...
    p.add_argument("--copy-to-thesis", action="store_true",
                   help="Copy output figures into thesis/Master/figs/design/gpu_v4/.")
    args = p.parse_args()
    ...
    for d in datasets:
        ...
        qps_out     = out_dir / f"qps_dsplit_vs_cpu_{d['key']}{suffix}.png"
        speedup_out = out_dir / f"speedup_dsplit_vs_cpu_{d['key']}{suffix}.png"
